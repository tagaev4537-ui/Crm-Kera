...
import { Button, Card, Input, Select, Modal, Spinner, EmptyState, Badge, Avatar } from "../components/ui.jsx";
import { CLIENT_STATUS_LABELS, formatDate } from "../utils/format.js";

const STATUS_OPTIONS = ["NEW", "IN_PROGRESS", "CLIENT", "LOST"];
const STATUS_TONE = { NEW: "accent", IN_PROGRESS: "warning", CLIENT: "success", LOST: "danger" };
...
export default function ClientsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="База клиентов"
        subtitle="Ведите лиды, активных покупателей и историю обращений."
        action={
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-ink-faint">
                  ...
                  <th className="px-5 py-3 font-medium">Статус и источник</th>
                  <th className="px-5 py-3 font-medium">Менеджер</th>
                  <th className="px-5 py-3 font-medium">Сделок</th>
                  ...
                  <tr key={c.id} className="border-b border-line last:border-0 hover:bg-bg/60">
                    <td className="px-5 py-3.5">
                      <Link to={`/clients/${c.id}`} className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-ink hover:text-accent-dark">{c.fullName}</span>
                      </Link>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <Badge tone={STATUS_TONE[c.status]}>{CLIENT_STATUS_LABELS[c.status] || c.status}</Badge>
                      {c.source && <div className="mt-1 text-xs text-ink-faint">{c.source}</div>}
                    </td>
                    <td className="px-5 py-3.5 text-ink-soft">{c.manager?.fullName}</td>
                    <td className="px-5 py-3.5 text-ink-soft">{c._count?.deals ?? 0}</td>
                    <td className="px-5 py-3.5 text-ink-faint">{formatDate(c.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
