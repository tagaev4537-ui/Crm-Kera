export function Button({ variant = "dark", className = "", ...props }) {
  const base =
    "inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-colors focus-ring disabled:opacity-50 disabled:cursor-not-allowed";
  const variants = {
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        } bg-surface px-3 py-2.5 text-sm text-ink placeholder:text-ink-faint focus-ring ${className}`}
        {...props}
      />
...
export function Select({ label, className = "", children, ...props }) {
  return (
    <label className="block">
      {label && <span className="mb-1.5 block text-sm font-medium text-ink-soft">{label}</span>}
      <select
        className={`w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink focus-ring ${className}`}
        {...props}
      >
        {children}
      </select>
...
export function Textarea({ label, className = "", ...props }) {
  return (
    <label className="block">
      {label && <span className="mb-1.5 block text-sm font-medium text-ink-soft">{label}</span>}
      <textarea
        className={`w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink placeholder:text-ink-faint focus-ring ${className}`}
        {...props}
      />
    </label>
  );
...
// Круглый аватар с инициалами - используется вместо фото сотрудника/клиента
export function Avatar({ name, size = 32, className = "" }) {
  const initials = (name || "?")
    ...
    .join("");
  const colors = ["bg-navy", "bg-accent-dark", "bg-ink-soft", "bg-success", "bg-danger"];
...
export function Modal({ open, onClose, title, children, wide = false }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4" onClick={onClose}>
      <div
        className={`max-h-[90vh] w-full ${wide ? "max-w-2xl" : "max-w-md"} overflow-y-auto rounded-2xl bg-surface p-6 shadow-card`}
